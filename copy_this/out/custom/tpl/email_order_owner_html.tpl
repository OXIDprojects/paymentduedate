  ...
  <body bgcolor="#FFFFFF" link="#355222" alink="#355222" vlink="#355222" style="font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 10px;">
    ...
    <table border="0" cellspacing="0" cellpadding="2" width="600">
	  ...
	  <tr>
		...
		[{ oxmultilang ident="ORDER_ALLOWANCE1"}][{$basket->sAllowanceDate}][{ oxmultilang ident="ORDER_ALLOWANCE_O"}][{ $basket->fPaymentAllowanceSum }][{ $currency->sign}][{ oxmultilang ident="ORDER_ALLOWANCE3"}][{$basket->fAllowancePrice}][{ $currency->sign}])
		</td>
	  </tr>
	  [{/if}]
	  [{/foreach}]
