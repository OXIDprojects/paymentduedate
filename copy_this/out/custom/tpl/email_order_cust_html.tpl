...
<html>
  ...
  <table border="0" cellspacing="0" cellpadding="2" width="600">
	...
	[{foreach from=$basket->aPaymentId4Allowance item=sPaymentAllowanceActive}]
	[{if $sPaymentAllowanceActive == $basket->getPaymentId() && $basket->dPaymentAllowanceSum !== 0}]
	<tr>
      <td height="0.5" bgcolor="#BEBEBE"></td>
      <td height="0.5" bgcolor="#BEBEBE"></td>
      <td height="0.5" bgcolor="#BEBEBE"></td>
      <td height="0.5" bgcolor="#BEBEBE"></td>
      <td height="0.5" bgcolor="#BEBEBE"></td>
	...
	<tr>
	    <td style="font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 10px;" valign="top" align="left" colspan="2">
		[{ oxmultilang ident="ORDER_ALLOWANCE1"}][{$basket->sAllowanceDate}][{ oxmultilang ident="ORDER_ALLOWANCE2"}][{ $basket->fPaymentAllowanceSum }][{ $currency->sign}][{ oxmultilang ident="ORDER_ALLOWANCE3"}][{$basket->fAllowancePrice}][{ $currency->sign}])
		</td>
	</tr>
	[{/if}]
	[{/foreach}]
  ...
  [{ if $order->oxorder__oxremark->value }]
    <br><b>[{ oxmultilang ident="EMAIL_ORDER_CUST_HTML_YOURMESSAGE" }] </b>[{ $order->oxorder__oxremark->value }]<br>
  [{/if}]

  <br>
  [{if $payment->oxuserpayments__oxpaymentsid->value != "oxempty"}][{ oxmultilang ident="EMAIL_ORDER_CUST_HTML_PAYMENTMETHOD" }] <b>[{ $payment->oxpayments__oxdesc->value }]</b><br>[{/if}]<br>
