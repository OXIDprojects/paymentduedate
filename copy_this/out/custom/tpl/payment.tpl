            ...
            [{if $oView->getPaymentList()}]
              ...
              <table class="form" style="width:92%">
                ...
                [{foreach key=sPaymentID from=$oView->getPaymentList() item=paymentmethod name=PaymentSelect}]
                  ...
                  [{elseif $sPaymentID == "oxidcreditcard"}]
                    ...
                    <tr onclick="oxid.form.select('paymentid',[{$inptcounter}]);">
                      ...
                      <td>
                        ...
                        [{foreach from=$creditYears item=year}]
				  ...
				  <tr>
					<td></td>
					<td colspan="2">[{foreach from=$paymentmethod->aPaymentId4Allowance item=sPaymentAllowanceActive}][{if $sPaymentAllowanceActive == $sPaymentID && $paymentmethod->fAddAllowanceSum !== '0' }][{ oxmultilang ident="PAYMENT_ALLOWANCE" }][{$paymentmethod->iAllowancePayDay}][{ oxmultilang ident="PAYMENT_ALLOWANCE2"}][{$paymentmethod->fAddAllowanceSum}][{ $currency->sign}][{ oxmultilang ident="PAYMENT_ALLOWANCE3"}][{/if}][{/foreach}]
					</td>
				   </tr>
